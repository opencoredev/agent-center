...
repoConnectionRoutes.get("/", async (context) => {
  ...
  return runApiEffect(context, tryApiPromise(() => repoConnectionService.list(filters)));
});

repoConnectionRoutes.post("/", async (context) => {
  const input = await validateJson(context, createRepoConnectionSchema);

  return runApiEffect(context, tryApiPromise(() => repoConnectionService.create(input)), 201);
});
...
repoConnectionRoutes.post("/:repoConnectionId/test", async (context) => {
  ...
  return runApiEffect(context, tryApiPromise(() => repoConnectionService.test(repoConnectionId)));
});

repoConnectionRoutes.get("/:repoConnectionId", async (context) => {
  const { repoConnectionId } = validateParams(context, repoConnectionIdParamsSchema);

  return runApiEffect(context, tryApiPromise(() => repoConnectionService.getById(repoConnectionId)));
});

repoConnectionRoutes.delete("/:repoConnectionId", async (context) => {
  const { repoConnectionId } = validateParams(context, repoConnectionIdParamsSchema);

  return runApiEffect(context, tryApiPromise(() => repoConnectionService.delete(repoConnectionId)));
});
